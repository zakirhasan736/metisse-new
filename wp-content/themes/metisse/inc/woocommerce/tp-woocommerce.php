...
add_action('woocommerce_single_product_summary', 'metisse_product_single_features', 60);

add_action('woocommerce_archive_description', 'woocommerce_breadcrumb', 20);

add_filter('metisse_woo_compare_filter', 'metisse_woo_compare_func');
    ...
    function vendor_more_products_tab($tabs)
    {
        if (check_more_seller_product_tab()) {
            $tabs['more_seller_product'] = [
                'title'    => __('More Products From Seller', 'metisse'),
                'priority' => 99,
...
?>
        <div class="row row-cols-xl-3 row-cols-lg-3 row-cols-sm-2 row-cols-1">
            <?php
            if ($products->have_posts()) {
                ...
                esc_html_e('No product has been found!', 'metisse');
            }
            ?>
        </div>
    <?php
...
// coupon
function metisse_woo_compare_func()
{
...
function metisse_product_single_features()
{

    $features_list = function_exists('tpmeta_field') ? tpmeta_field('metisse_product_single_fea_meta') : '';

    $payment_switch = get_theme_mod('metisse_product_single_payment_switch', false);
    $payment_text = get_theme_mod('metisse_product_single_payment_text', 'Guaranteed safe & secure checkout');
    $payment_img = get_theme_mod('metisse_product_single_payment_img');


    ?>

    <?php if (!empty($features_list[0]['metisse_product_signle_fea'])) : ?>
        <div class="tp-product-details-msg mb-15">
            <ul>
                <?php foreach ($features_list as $feature) : ?>
                    <li><?php echo metisse_kses($feature['metisse_product_signle_fea']); ?></li>
                <?php endforeach; ?>
            </ul>
        </div>
    <?php endif; ?>
    ...
    <?php if ($payment_switch == true) : ?>
        <div class="tp-product-details-payment d-flex align-items-center flex-wrap justify-content-between">
            ...
            <?php if (!empty($payment_img)) : ?>
                <img src="<?php echo esc_url($payment_img); ?>" alt="<?php echo esc_attr__('payment-img', 'metisse'); ?>">
            <?php endif; ?>
        </div>
    <?php endif; ?>
 ...
 *************************************************/
function metisse_shipping_progress_bar()
{
...
if (get_theme_mod('shipping_progress_bar_location_card_page', false) == true) {
    add_action('woocommerce_before_cart_table',  'metisse_shipping_progress_bar');
}

if (get_theme_mod('shipping_progress_bar_location_mini_cart', false) == true) {
    add_action('woocommerce_before_mini_cart_contents', 'metisse_shipping_progress_bar');
}

if (get_theme_mod('shipping_progress_bar_location_checkout', false) == true) {
    add_action('woocommerce_checkout_before_customer_details', 'metisse_shipping_progress_bar');
}
...
function metisse_sale_percentage()
{
    global $product;
    $output = '';
    $icon = esc_html__("-", 'metisse');
...
if (!function_exists('metisse_header_add_to_cart_fragment')) {
    function metisse_header_add_to_cart_fragment($fragments)
    {
        ob_start();
    ?>
...
add_filter('woocommerce_add_to_cart_fragments', 'metisse_header_add_to_cart_fragment');

// metisse_shopping_cart
function metisse_shopping_cart()
{
    if (is_null(WC()->cart)) {
        return;
    }
            ...
            <div class="cartmini__top-wrapper">
                <div class="cartmini__top p-relative">
                    <div class="cartmini__top-title">
                        <h4><?php print esc_html__('Shopping cart', 'metisse'); ?></h4>
                    </div>
                    ...
                    <div class="col">
                        <div class="tp-mobile-item text-center">
                            <a href="<?php echo esc_url(get_permalink(get_option('woocommerce_shop_page_id'))); ?>" class="tp-mobile-item-btn">
                                <i class="metisseicon-store"></i>
                                <span><?php echo esc_html__('Store', 'metisse'); ?></span>
                            </a>
                        </div>
                    </div>
                <?php endif; ?>
                <div class="col">
                    <div class="tp-mobile-item text-center">
                        <button class="tp-mobile-item-btn tp-search-open-btn">
                            <i class="metisseicon-magnifying-glass"></i>
                            <span><?php echo esc_html__('Search', 'metisse'); ?></span>
                        </button>
                    </div>
                </div>
                    ...
                    <div class="col">
                        <div class="tp-mobile-item text-center">
                            <a href="<?php echo esc_url($wishlist_data::get_url($key, true)); ?>" class="tp-mobile-item-btn">
                                <i class="metisseicon-heart"></i>
                                <span><?php echo esc_html__('Wishlist', 'metisse'); ?></span>
                            </a>
                        </div>
                    </div>
                <?php endif; ?>
                    ...
                    <div class="col">
                        <div class="tp-mobile-item text-center">
                            <a href="<?php echo get_permalink(get_option('woocommerce_myaccount_page_id')); ?>" class="tp-mobile-item-btn">
                                <i class="metisseicon-user"></i>
                                <span><?php echo esc_html__('Account', 'metisse'); ?></span>
                            </a>
                        </div>
                    </div>
                <?php endif; ?>
                <div class="col">
                    <div class="tp-mobile-item text-center">
                        <button class="tp-mobile-item-btn tp-offcanvas-open-btn">
                            <i class="metisseicon-menu-1"></i>
                            <span><?php echo esc_html__('Menu', 'metisse'); ?></span>
                        </button>
                    </div>
                </div>
            </div>
...
add_filter('woocommerce_single_product_image_thumbnail_html', 'metisse_product_img', 10, 2);
function metisse_product_img($html, $id)
{

    $video_url = function_exists('tpmeta_field') ? tpmeta_field('metisse_product_signle__video') : '';
...
if (!function_exists('metisse_content_single_details')) {
    function metisse_content_single_details()
    {
        global $product;
        global $post;
        global $woocommerce;
        ...
        $enable_category = get_theme_mod('metisse_product_single_category_switch', true);
        $enable_stock = get_theme_mod('metisse_product_single_in_stock_switch', true);
        $enable_flash_sale = get_theme_mod('metisse_product_single_flash_sale_switch', false);
        $enable_flash_sale_text = get_theme_mod('metisse_product_single_flash_sale_text', 'Flash Sale Ends in:');
        $enable_stock_left = get_theme_mod('metisse_product_single_stock_left_switch', '49');
  ...
  <path d="M16.708 10.0792C16.8535 10.3741 17.1347 10.5785 17.46 10.6261L23.5179 11.5115L19.1353 15.7802C18.8994 16.01 18.7917 16.3411 18.8474 16.6656L19.8815 22.6949L14.4651 19.8465C14.1737 19.6933 13.8256 19.6933 13.5342 19.8465L8.11786 22.6949L9.15195 16.6656C9.20761 16.3411 9.09995 16.01 8.86407 15.7802L4.48142 11.5115L10.5393 10.6261C10.8647 10.5785 11.1458 10.3741 11.2914 10.0792L13.9997 4.59251L16.708 10.0792Z" fill="white" stroke="#FFBD3E" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <?php if (!empty($enable_flash_sale)) : ?>
            <?php if (!is_null($product->get_date_on_sale_to())) : ?>
                ...
                <div class="tp-product-details-countdown d-flex align-items-center justify-content-between flex-wrap">
                    ...
                    <div class="tp-product-details-countdown-time" data-countdown data-date="<?php echo esc_attr($_sale_date_end); ?>">
                        <ul>
                            <li><span data-days><?php echo esc_html__('0', 'metisse'); ?></span><?php echo esc_html__('D', 'metisse'); ?></li>
                            <li><span data-hours><?php echo esc_html__('0', 'metisse'); ?></span><?php echo esc_html__('H', 'metisse'); ?></li>
                            <li><span data-minutes><?php echo esc_html__('0', 'metisse'); ?></span><?php echo esc_html__('M', 'metisse'); ?></li>
                            <li><span data-seconds><?php echo esc_html__('0', 'metisse'); ?></span><?php echo esc_html__('S', 'metisse'); ?></li>
                        </ul>
                    </div>
                </div>
        ...
        <?php if (!empty($enable_stock_left)) : ?>
            ...
            ?>
                <div class="tp-product-details-stock-bar">
                    <p><?php echo esc_html__('Hurry! Only', 'metisse'); ?> <span><?php echo esc_html($stock_quantity); ?></span> <?php echo esc_html__('units left in stock!', 'metisse'); ?> </p>
...
add_action('woocommerce_single_product_summary', 'metisse_content_single_details', 4);



// product-content archive
if (!function_exists('metisse_content_product_grid')) {
    function metisse_content_product_grid()
    {


        if (get_theme_mod('shop_grid_layout') === "2") {
            metisse_product_grid_type_2();
        } elseif (get_theme_mod('shop_grid_layout') === "3") {
            metisse_product_grid_type_3();
        } elseif (get_theme_mod('shop_grid_layout') === "4") {
            metisse_product_grid_type_4();
        } elseif (get_theme_mod('shop_grid_layout') === "5") {
            metisse_product_grid_type_5();
        } elseif (get_theme_mod('shop_grid_layout') === "6") {
            metisse_product_grid_type_6();
        } else {
            metisse_product_grid_type_1();
        }
    }
}
add_action('woocommerce_before_shop_loop_item', 'metisse_content_product_grid', 10);


add_action('metisse_product_badge_action', 'metisse_product_badges');

function metisse_product_badges()
{
    global $product;
    global $post;
    global $woocommerce;
    ...
    $is_product_on_trending = function_exists('tpmeta_field') ? tpmeta_field('metisse_product_on_trending') : '';
    $is_product_on_hot = function_exists('tpmeta_field') ? tpmeta_field('metisse_product_on_hot') : '';

    $product_bage_from_meta = function_exists('tpmeta_field') ? tpmeta_field('metisse_product_bage_type') : '';
    ...
    <ul class="d-flex">
        ...
        <?php if ($enable_trending_badge || $is_product_on_trending === "on") : ?>
            ...
            <?php if ($is_product_on_trending === "on") : ?>
                <li>
                    <span class="onsale on-trending"><?php echo esc_html__('Trending', 'metisse'); ?></span>
                </li>
        ...
        <?php if ($is_product_on_hot === "on") : ?>
            <li>
                <span class="onsale on-hot"><?php echo esc_html__('Hot', 'metisse'); ?></span>
            </li>
...
add_action('metisse_product_countdown_action', 'metisse_product_countdown');

function metisse_product_countdown()
{
    global $product;
    global $post;
    global $woocommerce;

    $enable_flash_sale = get_theme_mod('metisse_product_sale_countdown_switch', true);
...
function metisse_product_grid_type_1()
{
        ...
        <div class="product--card-main-cont flex items-start gap-[20px]">
            ...
            <div class="product--card-cont pt-5 pb-[22px]">
                ...
                <div class="product--features-watches-btn-box text-left relative left-0 w-full h-full flex gap-[25px] items-start justify-start">
                    ...
                    <button class="add-to-cart-btn mx-auto flex items-center justify-center w-full whitespace-nowrap h-[40px] py-2 px-8 bg-[#131313] border-0 text-[#FFFDFD] text-[11px] font-semibold uppercase text-center font-secondary leading-[1.1] tracking-[1.1px]"> <?php woocommerce_template_loop_add_to_cart('Add To Basket', 'metisse'); ?></button>
                </div>
            </div>
        </div>
    </div>
...
function metisse_product_grid_type_2()
{
    global $product;
    global $post;
    global $woocommerce;
        ...
        <?php if (has_post_thumbnail()) : ?>
            <div class="tp-product-thumb p-relative fix m-img">
                ...
                <div class="tp-product-badge ">
                    <?php do_action('metisse_product_badge_action'); ?>
                </div>
                    ...
                    <div class="tp-product-action-item d-flex flex-column">
                        ...
                        <?php if (function_exists('woosc_init')) : ?>
                            ...
                            <div class="tp-product-action-btn tp-woo-add-to-compare-btn tp-woo-action-btn">
                                ...
                                <span class="tp-product-tooltip tp-woo-tooltip"> <?php echo esc_html__('Add To Compare', 'metisse'); ?></span>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
...
function metisse_product_grid_type_3()
{
    global $product;
    global $post;
    global $woocommerce;
...
function metisse_product_grid_type_4()
{
    global $product;
    global $post;
    global $woocommerce;
...
function metisse_product_grid_type_5()
{
    global $product;
    global $post;
    global $woocommerce;
...
function metisse_product_grid_type_6()
{
    global $product;
    global $post;
    global $woocommerce;
                    ...
                    <div class="tp-product-action-item-2 d-flex flex-column">
                        ...
                        <?php if (function_exists('woosc_init')) : ?>
                            ...
                            <div class="tp-product-action-btn-2 tp-woo-add-to-compare-btn tp-woo-action-btn">
                                <?php echo do_shortcode('[woosc]'); ?>
                                <span class="tp-product-tooltip tp-product-tooltip-right tp-woo-tooltip"> <?php echo esc_html__('Add To Compare', 'metisse'); ?></span>
                            </div>
                        <?php endif; ?>
...
{
    // smart quick view
    add_filter('woosq_button_html', 'metisse_woosq_button_html', 10, 2);
    function metisse_woosq_button_html($output, $prodid)
    {
        $btntext = esc_html__("Quick View", 'metisse');
    ...
    // smart wishlist
    add_filter('woosw_button_html', 'metisse_woosw_button_html', 10, 3);

    function metisse_woosw_button_html($output, $id, $attrs)
    {
    ...
    add_filter('woosw_button_text', 'metisse_woosw_button_text', 10, 1);
    function metisse_woosw_button_text($output)
    {
        $btntext = esc_html__("Add To Wishlist", 'metisse');
        $action_layout = get_theme_mod('shop_grid_layout');
        if ($action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5") {
            $tooltip_position = '';
        } else {
    ...
    add_filter('woosw_button_text_added', 'metisse_woosw_button_text_added', 10, 1);
    function metisse_woosw_button_text_added($output)
    {

        $btntext = esc_html__("View Wishlist", 'metisse');
        $action_layout = get_theme_mod('shop_grid_layout');
        if ($action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5") {
            $tooltip_position = '';
        } else {
    ...
    add_filter('woosc_button_htmls', 'metisse_woosc_button_html', 10, 2);
    function metisse_woosc_button_html($output, $id)
    {
        global $product;
        $product = wc_get_product($id);
        $product_name = $product ? $product->get_name() : '';
    ...
    add_filter('woosc_button_texts', 'metisse_woosc_button_text', 10, 1);
    function metisse_woosc_button_text($output)
    {
        $btntext = esc_html__("Add To Compare", 'metisse');
        $action_layout = get_theme_mod('shop_grid_layout');
        if ($action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5") {
            $tooltip_position = '';
        } else {
    ...
    add_filter('woosc_button_text_addeds', 'metisse_woosc_button_text_added', 10, 1);

    function metisse_woosc_button_text_added($output)
    {
        $btntext = esc_html__("View Compare", 'metisse');
        $action_layout = get_theme_mod('shop_grid_layout');
        if ($action_layout === "2" ||  $action_layout === "3" || $action_layout === "4" || $action_layout === "5") {
            $tooltip_position = '';
        } else {
...
// product-content
if (!function_exists('metisse_content_product_list')) {
    function metisse_content_product_list()
    {
        global $product;
        global $post;
        global $woocommerce;
        ...
        <div class="tp-product-list-item d-md-flex tp-woo-product-list-item">
            ...
            <div class="tp-product-list-content">
                <div class="tp-product-content-2 pt-15">
                    ...
                    <h3 class="tp-product-title-2 leading-none mb-[7px] text-[18px] text-left font-primary font-bold capitalize text-black">
                        <a class="leading-none mb-[7px] text-[18px] text-left font-primary font-bold capitalize text-black" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                    </h3>
...
function woocommerce_template_loop_add_to_cart($args = array())
{
    ...
    if ($product->is_type('simple')) {
        ...
        $btntext = esc_html__("Buy Now", 'metisse');
    ...
    function woocommerce_breadcrumb($args = array())
    {
        $args = wp_parse_args(
            ...
            apply_filters(
                ...
                array(
                    'delimiter'   => '',
                    'wrap_before' => '<nav class="woocommerce-breadcrumb tp-woo-breadcrumb-wrapper">',
                    'wrap_after'  => '</nav>',
                    'before'      => '',
                    'after'       => '',
                    'home'        => _x('Home', 'breadcrumb', 'metisse'),
                )
            )
        );
        ...
        // Output the breadcrumb without the wrapping elements
        echo $args['wrap_before'] . '<ul class="tp-woo-breadcrumb-list">';
        foreach ($args['breadcrumb'] as $key => $crumb) {
            ...
            if (!empty($crumb[1]) && sizeof($args['breadcrumb']) !== $key + 1) {
                ...
                echo '<span>/</span>';
            } elseif (!empty($crumb[1]) && sizeof($args['breadcrumb']) === $key + 1) {
                echo '<a href="' . esc_url($crumb[1]) . '">' . esc_html($crumb[0]) . '</a>';
            } else {
...
{
    global $product;
    $enable_buy_now = get_theme_mod('metisse_product_single_buy_now_switch', true);
    if ($enable_buy_now) {
        printf('<button id="tp_wc-adding-button" type="submit" name="tp-wc-buy-now" value="%d" class="tp-product-details-buy-now-btn w-100 single_add_to_cart_button buy_now_button button alt">%s</button>', $product->get_ID(), esc_html__('Buy Now', 'metisse'));
    }
}
