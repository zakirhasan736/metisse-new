...
class TP_El_Woocommerce
{
        ...
        global $product;
            ...
            if ( $product ) {
             ...
             if( $product->is_type( 'simple' ) ){
                $btntext = esc_html__("Add To Basket",'harry');
    ...
    public static function add_to_cart_button($product_id)
    {
        ...
        if ($product) {
            ...
            return sprintf(
                '<a rel="nofollow" href="%s" data-quantity="%s" data-product_id="%s" data-product_sku="%s" class="%s add_to_cart_button tp-btn"><i class="fal fa-shopping-cart"></i> Add To Basket</a>',
                esc_url($product->add_to_cart_url()),
