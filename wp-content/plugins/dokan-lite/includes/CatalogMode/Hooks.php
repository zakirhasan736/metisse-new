...
class Hooks {
    ...
    /**
     * This method will hide Add to basket button for products if enabled by vendor
     *
     * @since 3.6.4
     *
    ...
    public function hide_add_to_cart_button( $purchasable, $product ) {
        ...
        if ( Helper::is_enabled_for_product( $product ) ) {
            return false; // per product settings to hide Add to basket button is enabled
        }
        ...
        if ( Helper::is_enabled_by_vendor( $vendor_id ) ) {
            return false; // vendor global settings to hide Add to basket button is enabled
        }
    ...
    public function hide_product_price( $price, $product ) {
        // for admin panel, we don't need to hide Add to basket button
        if ( is_admin() ) {
            return $price;
        }
        ...
        // check if hide Add to basket button is enabled by product
